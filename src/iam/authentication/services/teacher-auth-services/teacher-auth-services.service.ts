...
export class TeacherAuthService {
  ...
  async signup(body: CreateUser) {
    try {
      ...
      return { accessToken, user: { newTeacher } };
    } catch (error) {
  ...
  async forgotPassword(body: ForgotPasswordDTO) {
    try {
      const { email } = body;
      const user = await this.teacherRepo.findOneBy({ email: email });
      ...
      await this.teacherRepo.save(user);
  ...
  async resetPassword(token: string, body: ResetPasswordDTO) {
    ...
    const user = await this.teacherRepo.findOneBy({ resetToken: token });
  ...
  async updatePassword(body: UpdatePasswordDTO, ActiveUser: ActiveUserDTO) {
    ...
    const user = await this.teacherRepo.findOneBy({
      id: +ActiveUser.sub,
    });
